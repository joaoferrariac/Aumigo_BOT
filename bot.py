...
def clear_chat():
    """
    Limpa todo o conteúdo da janela de bate-papo.
    """
    Chat.config(state=NORMAL)
    Chat.delete("1.0", END)
    Chat.config(state=DISABLED)
...
ClearButton = Button(base, font=("Verdana", 10, 'bold'), text="Limpar", width="12", height=2, bd=0, 
                    bg="#f00", activebackground="#a00", fg='#ffffff', command=clear_chat)

ClearButton.place(x=128, y=460, height=50, width=260)
...
base.bind('<Return>', enter_pressed)
