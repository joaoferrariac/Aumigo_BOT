...
# Após o lematizador
def normalize_gender(sentence_words):
    """
    Normaliza variações de gênero em palavras comuns no português.
    """
    gender_map = {
        "obrigado": "obrigada",  # Mapear palavras no masculino para o feminino
    }
    return [gender_map.get(word, word) for word in sentence_words]

# Substitua a função clear_writing:
def clear_writing(writing):
    """
    Limpa e normaliza a frase inserida.
    """
    sentence_words = nltk.word_tokenize(writing)
    lemmatized = [lemmatizer.lemmatize(word.lower()) for word in sentence_words]
    return normalize_gender(lemmatized)
